...
func update_asteroid_from_resource(astro_resource) -> void:
	...
	astro_rigid_body.move_direction_vector = move_direction_vector
	astro_rigid_body.mass = update_asteroid_mass(asteroid_size)
	rigid_body_collider.shape.radius = radius_var-30
